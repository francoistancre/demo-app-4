...
class JoueurControleur:
    ...
    def rapport_liste_joueur(self, ordre):
        ...
        if ordre == "alphabetique":
            liste_joueurs_sorted = sorted(
                liste_joueurs, key=lambda d: d["nom"], reverse=False
            )
            self.rapport_vue.get_rapport_data(
                liste_joueurs_sorted, "Liste des joueurs par ordres alphabétique"
            )
        elif ordre == "classement":
            liste_joueurs_sorted = sorted(
                liste_joueurs, key=lambda d: d["classement"], reverse=True
            )
            self.rapport_vue.get_rapport_data(
                liste_joueurs_sorted, "Liste des joueurs par classement"
            )
