...
class TourControleur:
    ...
    def tour_1(self, index_tournoi, index_joueurs):
        ...
        #appeler save tour
        tour = Tour(index_tour, index_tournoi, name_tour, liste_matchs, date_debut, date_fin)
        tour.save_tours()    
    ...
    def test_match(self, liste_matchs, new_match):
        ...
        for match in liste_matchs:
            ...
            if (new_joueur_1 == old_joueur_1 or new_joueur_1 == old_joueur_2) and (new_joueur_2 == old_joueur_1 or new_joueur_2 == old_joueur_2):
                compteur += 1
        if compteur != 0:
            #print("ce match a deja été joué")
            return False 
        else:
            #print("ce match n'a pas été joué")
            return True
    ...
    def test_joueur(self, liste_matchs, joueur):
        ...
        for match in liste_matchs:
            ...
            if (joueur == old_joueur_1 or joueur == old_joueur_2):
                compteur += 1
        if compteur != 0:
            #print("ce joueur a deja joué")
            return False 
        else:
            #print("ce joueur n'a pas joué")
            return True
    ...
    def tour(self, index_tournoi, liste_matchs, numero_tour, index_joueurs):
        ...
        liste_joueurs = self.manager.recuperer_joueur_tournoi(index_joueurs)
        new_liste_joueurs = sorted(liste_joueurs, key=lambda d: d['score'], reverse=True)
        new_liste_matchs = []

        new_liste_matchs = self.tour_generate(new_liste_joueurs,new_liste_matchs,liste_matchs)
        ...
        date_fin = str(datetime.datetime.now())
        tour = Tour(index_tour, index_tournoi, name_tour, new_liste_matchs, date_debut, date_fin)
        tour.save_tours()  

    def continuer_tour(self, index_tournoi, index_joueurs):
        tour_list = self.manager.recuperer_tours(index_tournoi)
        if len(tour_list) != 0:
            #print("Il y a au moins un tour")
            #print(tour_list)
            ...
            if nombre_de_tours == 1:
                ...
                matchs_list = tour_list[0]["matchs"] + tour_list[1]["matchs"] + tour_list[2]["matchs"]
    ...
    def afficher_tour(self):
        ...
        index_joueurs = self.manager.search_tournoi_index(index_tournoi)[0]["index_joueurs"]
        print(index_joueurs)
        ...
        #self.continuer_tour(index_tournoi, tour_list, index_joueurs)
